...
for s = 1:length(stageList)
    ...
    %% ---- Remove invalid rows ----
    valid = ~isnan(groupIdx) & ~isnan(PctCorr);

    groupIdx   = groupIdx(valid);
    PctCorr    = PctCorr(valid);
    ...
    %% ---- Formatting ----
    ylabel('Proportion of wave-associated calcium transients per ROI (%)');
